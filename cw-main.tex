...
}
{
   ...
   когда сумма квадратов его противоположных сторон равна квадрату диаметра описанной окружности (Рис.~\ref{t6:im}).
   Таким образом, требуется доказать, что
   \begin{equation}
   ...
   \begin{figure}[h]
      \centering
      \includegraphics[width=0.7\textwidth]{images/task6.pdf}
      \caption{Иллюстрация к задаче}
      \label{t6:im}
   \end{figure}
   \paragraph{Решение задачи:}
   ...
   Почленно вычтя второе уравнение системы (\ref{t6:f3}) из первого, получаем:
   \begin{flalign*}
      \label{t6:f4}
   ...
   \end{flalign*}
   Таким образом, утверждение (\ref{t6:f1}) доказано.
   \paragraph{Вычислительная иллюстрация на частном случае:} На вход программы передаются координаты
   свободных точек комплексной плоскости, в данном примере это координаты точек \(A,B,C,D\), по ним
   строится окружность. Если условие (\ref{t6:f1}) выполняется (вместо \(PB\) используется конструкция
   ...
   на Рис. \ref{t6:scheme}.
   \begin{center}
      \includegraphics[width=0.79\textwidth]{images/task6-diagram.png}
      \captionof{figure}{Блок-схема алгоритма программы}
      \label{t6:scheme}
   \end{center}
   ...
   \lstinputlisting[frame=none,caption={Функция task6::solve},label=task6-code,captionpos=b,firstline=5]{\taskFileName{6}}
   \paragraph{Демонстрация работы:}
   TODO % TODO
}
{
   ...
   \lstinputlisting[frame=none,caption={Функция task7::solve},label=task7-code,captionpos=b,firstline=6]{\taskFileName{7}}
   \paragraph{Демонстрация работы:}
   TODO % TODO
}
\tcchapter{Заключение}
